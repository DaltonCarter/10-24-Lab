...
function thatsOdd(num1){
  if(num1 % 2 === 0){
    ...
    return 'How about you put in a number dude.'
  }
}

let oddChecker = thatsOdd(78)
...
//CODE HERE
 ...
 bestMovie = (name) => `${name} is the best movie ever!`

 
 let response = bestMovie('The Fifth Element')
...
function emailCheck(email){
  let blaarg = email.trim()
  if(blaarg.includes('@')){
    return `email verified ${blaarg}.`
  }else {
    return ' Must provide a valid email address.'
  }
...
let emailRespose = emailCheck('         sudra@yahoo.com')
...
function snacks2(gold){
  if(gold / 3){
    return Math.trunc(gold / 3)
  }
}

let totalFrogs2 = snacks2(500)
...
let arrayIsAscending = bigOof(sampleArray)
